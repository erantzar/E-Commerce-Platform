...
export const updateAddress = async (req, res) => {
    try {
        ...
        const address = user.addresses._id(addrId);
        if (!address)
...
export const addUserAdress = async (req, res) => {
    try {
        const { id } = req.user;
        const { city, street, houseNumber, zip } = req.body;
        const user = await User.findByIdAndUpdate({ _id: id }, { $push: { addresses: { city, street, houseNumber, zip } } }, { new: true, runValidators: true });
        if (!user)
            throw new Error("User not found");
        res.status(200).json({
            status: 200,
