...
export const createOrder = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { items, addressId, paymentMethod, notes, shipingCost } = req.body as {
    items: any[]; // שימוש ב-any כאן מקל על ה-loop
    addressId: string;
  ...
  const resolvedItems: any[] = []; // שינוי ל-any[] פותר את שגיאת ה-Overload ב-create
  let totalPrice = 0;

  for (const orderItem of items) {
    const product = await Product.findById(orderItem.product).select(
      "stock price name images isActive sold"
    );
  ...
  await sendOrderEmail(order as any,email);
 ...
 * @access  Confirmed User
 */
export const myOrders = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.user!;
 ...
 */
export const singelOrderById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
if (customerEmail) {
    ...
    await sendOrderEmail(order as any,customerEmail);
}
